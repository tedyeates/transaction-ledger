...
export function TransactionTable({
  ...
  const columns = [
    ...
    { key: 'branch',        label: 'สาขา',        filterKey: 'colBranch',    numeric: false },
    { key: 'location',      label: 'ที่ตั้ง',      filterKey: 'colLocation',  numeric: false },
    { key: 'terminal_id',   label: 'รหัสเครื่อง',  filterKey: 'colTerminal',  numeric: false },
    { key: 'narrative',     label: 'หมายเหตุธนาคาร', filterKey: 'colNarrative', numeric: false },
    ...
    ...(role === ROLES.admin ? [{ key: 'counterparty_account', label: 'เลขที่บัญชีคู่ค้า', numeric: false }] : []),
    ...(role === ROLES.admin ? [{ key: 'fx_rate', label: 'อัตราแลกเปลี่ยน', numeric: true }] : []),
    ...(role === ROLES.admin ? [{ key: 'statement_format', label: 'รูปแบบใบแจ้งยอด', numeric: false }] : []),
    ...(role === ROLES.admin ? [{ key: 'statement_exported_at', label: 'เวลาส่งออกใบแจ้งยอด', numeric: false }] : []),
