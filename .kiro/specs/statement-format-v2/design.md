...
- `texport.csv` stays a local, gitignored file at the repo root — it is **never committed**. It contains masked account numbers and Thai company names from a real statement; the masking is the bank's (`0942XXX148`). Treat it as real data: keep it to the 92 rows already present rather than adding more, and never remove it from `.gitignore`. Tests that read it must guard on its absence and skip cleanly when it is not present locally.
